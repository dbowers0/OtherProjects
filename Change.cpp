...
def algorithmChoice() :
	...
	choice = input("Please enter choice of algorithm you would like to view the time complexities for or enter <F> to quit: \n")
					...
					while loop :
						...
						if choice == "A" or choice == "a" : #average case for bubble sort
							...
							for x in range(1, 5) :
								...
								averageBubbleSortedList = bubbleSort(m)
								bt2 = time.perf_counter()
								print('Bubble sort Average Case', k, 'items costs', round((bt2 - bt1), 5), 'seconds')
								with open("averageBubbleTimes.txt", "a") as average_bubble_times :
				average_bubble_times.write(f"{round((bt2-bt1), 8)}\n")

					k *= 10
					intk = int(k)

# elif choice == "B" or choice == "b":  #worst case for bubble sort, numbers are in descending order
#for loop nums - 10, 100, 1000, 10000
#    for x in range(1, 5) :
	#       m = random.sample(range(10 * intk), intk)
...
for x in range(1, 5) :
	...
	selectionSortedList = selectionSort(m)
	st2 = time.perf_counter()
	print('Selection sort', k, 'items costs', round((st2 - st1), 5), 'seconds')
